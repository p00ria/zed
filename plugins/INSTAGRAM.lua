...
local function instagramUser(msg, query)
	...
	if user.data.bio ~= '' then
		text = text.."یوزر: "..user.data.username:upper().."\n\n"
	else
		text = text.."یوزر: "..user.data.username:upper().."\n"
	end
	if user.data.bio ~= '' then
	...
	text = text.."تعداد پست: "..user.data.counts.media.."\n"
	text = text.."دنبال کننده: "..user.data.counts.follows.."\n"
	text = text.."دنبال کنندگان: "..user.data.counts.followed_by.."\n"
